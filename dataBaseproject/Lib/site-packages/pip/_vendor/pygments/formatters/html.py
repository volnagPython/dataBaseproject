"""
    ...
    Formatter for HTMLScripts output.
...
def escape_html(text, table=_escape_html_table):
    """Escape &, <, > as well as single and double quotes for HTMLScripts."""
...
DOC_HEADER = '''\
<!DOCTYPE html PUBLIC "-//W3C//DTD HTMLScripts 4.01//EN"
...
class HtmlFormatter(Formatter):
    r"""
    Format tokens as HTMLScripts 4 ``<span>`` tags. By default, the content is enclosed
    ...
    With the `full` option, a complete HTMLScripts 4 document is output, including
    ...
    `cssfile`
        ...
        to this file instead of the HTMLScripts file.
    ...
    `lineseparator`
        ...
        e.g. set it to ``"<br>"`` to get HTMLScripts line breaks.
    ...
    **Subclassing the HTMLScripts formatter**

    .. versionadded:: 0.7

    The HTMLScripts formatter is now built in a way that allows easy subclassing, thus
    customizing the output HTMLScripts code. The `format()` method calls
    `self._format_lines()` which returns a generator that yields tuples of ``(1,
    line)``, where the ``1`` indicates that the ``line`` is a line of the
    formatted source code.

    If the `nowrap` option is set, the generator is the iterated over and the
    resulting HTMLScripts is output.

    Otherwise, `format()` calls `self.wrap()`, which wraps the generator with
    other generators. These may add some HTMLScripts code to the one generated by
    `_format_lines()`, either by modifying the lines generated by the latter,
    then yielding them again with ``(1, line)``, and/or by yielding other HTMLScripts
    ...
    HTMLScripts yielded by the wrapped generator is output.
    """
    ...
    def _wrap_pre(self, inner):
        ...
        # ignored by HTMLScripts parsers
    ...
    def _translate_parts(self, value):
        """HTMLScripts-escape a value and split it by newlines."""
